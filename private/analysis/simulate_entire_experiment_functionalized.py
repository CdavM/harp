...
def load_sample_people_from_file(filename_forloadingpeople, superdictionary_forloadingpeople, deficit_offset_forloadingpeople, make_deficit_ideal_neg_infin=False, addeducationoffset=False):
    ...
    persons = []
    for mech in superdictionary_forloadingpeople:
        if not superdictionary_forloadingpeople[mech]['type'] == 'full':
            continue
        for row in organized_data[mech]:
            ideals = [row['question_data']['slider' +
                str(slider) + '0_loc'] for slider in range(5)]
            if make_deficit_ideal_neg_infin:  # ie have marginal disutility for any deficit
                ideals[4] = -1000.0
            if addeducationoffset:
...
def do_mechanism(turker_idealpt, mechanism_assignment,
                 mechanisms, time_of_last_turker, turker_time, mechanism_super_dictionary, deficit_offset, radius_parameters, current_avg_number, onesided = False, quadratic = False):
    ...
    row = simulate_person(turker_idealpt, mechanism_assignment, mechanisms,
                          row, mechanism_super_dictionary, deficit_offset, current_avg_number, onesided = onesided, quadratic = quadratic)
...
def find_ideal_pt_for_person_in_ball(center, radius, idealpt_and_radius, constraint="l1", deficit_offset=228, onesided = False, quadratic = False):
    X = cvxpy.Variable(5)  # 1 point for each item
    fun = 0
    y = copy.deepcopy(idealpt_and_radius[0]);
    w = idealpt_and_radius[1];
    sumsq = math.sqrt(sum([math.pow(max(w[i], .001), 2) for i in range(5)]))
    if sumsq < .01:
        print center, radius, idealpt_and_radius
    w = [(w[i]/ sumsq)*10.0 for i in range(5)]; #for larger numbers for cvx; also perturb so not exactly the same

    y[3] = -y[3]; #hardcode negative for the income tax to treat as expenditure
    cen = copy.deepcopy(center)
    cen[3] = -cen[3]

    if quadratic:
        for slider in range(5):
            fun += w[slider] * cvxpy.norm(X[slider] - y[slider], 2)

    elif onesided == False:
        for slider in range(5):
            # fun += w[slider] * cvxpy.norm(X[slider] - y[slider], 1)
            fun += w[slider] *cvxpy.abs(X[slider] - y[slider])
    else:
        for slider in range(4): # for expenditures
            fun += w[4]*X[slider] + w[slider] * cvxpy.pos(y[slider] - X[slider])
        #     if w[4] > w[slider]: #if deficit more important than slider, then decrease all the way
        #         fun += (w[4])*cvxpy.pos(X[slider] - y[slider]) + (w[4] - w[slider])*cvxpy.pos(y[slider] - X[slider])
        #     elif w[4] < w[slider]: #if deficit less important than slider, then care if below ideal pt; if above ideal pt, decrease with weight of deficit
        #         fun += (w[slider] - w[4])*cvxpy.pos(y[slider] - X[slider]) + (w[4])*cvxpy.pos(X[slider] - y[slider])
        # # #for income tax:
        # if w[4] > w[3]: #if deficit more important than slider, then decrease all the way
        #     fun += (w[4])*X[3] +w[3]*cvx.pos(y[3] - X[3])# + (w[4] - w[3])*cvxpy.pos(y[3] - X[3])
        # elif w[4] < w[3]: #if deficit less important than slider, then care if below ideal pt; if above ideal pt, decrease with weight of deficit
        #     fun += (w[3] - w[4])*cvxpy.pos(y[3] - X[3]) + (w[4])*cvxpy.pos(X[3] - y[3])
        # if w[4] > w[3]:
        #     #deficit weight is more important; so increase tax (decrease X) regardless of X, but with different weights
        #     #with only deficit weight if currently below ideal tax (X > y)
        #     #with deficit-tax weight if above ideal tax (X < y)
        #     fun += (w[4])*cvxpy.pos(X[slider] - y[slider]) + (w[4] - w[slider])*cvxpy.pos(y[slider] - X[slider])

        # #for income Tax
        # if w[4] > w[3]: #if deficit more important than slider, then increase tax
        #     fun += -(w[4] - w[3])*X[3]
        # elif w[4] < w[3]: #if deficit less important than slider, then care if above ideal pt
        #     fun += (w[3] - w[4])*cvxpy.pos(X[3] - y[3])
    # fun += w[4] * (X[4] - yp
    # print fun
    obj = cvxpy.Minimize(fun)
    constraints = [X[0:3] >= 0, X[3]<=0, X[0] + X[1] + X[2] + X[3] + deficit_offset == X[4]]

    if constraint == "l1":
        constraints += [cvxpy.sum_entries(cvxpy.abs(X[0:4] - cen[0:4])) <= radius]
    elif constraint == 'l2':
        constraints += [cvxpy.sum_entries(cvxpy.square(X[0:4] - cen[0:4])) <= radius**2]
    elif constraint == 'linf':
        # print "constraint type is linf"
        constraints += [cvxpy.max_entries(cvxpy.abs(X[0:4] - cen[0:4])) <= radius]
    # print fun
    prob = cvxpy.Problem(obj, constraints)
    if quadratic:
        result = prob.solve('SCS',max_iters = 30000)
    else:
        result = prob.solve()
    #print y, cen, result, X
    ...
    # if quadratic:
    # print w, center, y, radius, items
    if prob.status != 'optimal':
        print prob.status
    # # print idealpt_and_radius
    # # print center, radius, items
    # raise Exception('hi')
...
def simulate_person(idealpts, mechanism_assignment, mechanisms, row, mechanism_super_dictionary, deficit_offset=228, current_avg_number=0, onesided = False, quadratic = False):
    ...
    if mechanisms[mechanism_assignment]['question_ID'] == 'comparisons':
        ...
        for setnum in range(mechanism_super_dictionary[mechanism_assignment]['numsets']):
            values, credits = find_ideal_pt_for_person_in_ball(mechanisms[mechanism_assignment]['values'][
                                                               setnum], row['radius'], idealpts, constraint="l2", deficit_offset=deficit_offset, onesided = onesided, quadratic = quadratic)
            setstr = str(setnum)
            for i in range(0, 4):
        ...
        for setnum in range(mechanism_super_dictionary[mechanism_assignment]['numsets']):
            setstr = str(setnum)
            values, credits = find_ideal_pt_for_person_in_ball(mechanisms[mechanism_assignment]['values'][
                                                               setnum], row['radius'], idealpts, constraint="l1", deficit_offset=deficit_offset, onesided = onesided, quadratic = quadratic)
            for i in range(0, 4):
                answer["slider" + str(i) + setstr] = [str(values[i])];
                row['answer1.slider' + str(i) + setstr + "_credits"] = credits[i];
        ...
        for setnum in range(mechanism_super_dictionary[mechanism_assignment]['numsets']):
            setstr = str(setnum)
            values, credits = find_ideal_pt_for_person_in_ball(mechanisms[mechanism_assignment]['values'][
                                                               setnum], row['radius'], idealpts, constraint="linf", deficit_offset=deficit_offset, onesided = onesided, quadratic = quadratic)
            for i in range(0, 4):
                answer["slider" + str(i) + setstr] = [str(values[i])];
                row['answer1.slider' + str(i) + setstr + "_credits"] = credits[i];
...
def assignemnt_with_averaging(mechanisms, turker_time, mechanism_super_dictionary, TIME_IN_MECHANISM, probabilities=None):
    ...
    if all_busy:
        ...
        mechanism = np.random.choice(
            range(len(mechanism_super_dictionary.keys())), p=probabilities)
        while mechanism!=0 and check_busy_times(mechanisms[mechanism]['averaging_status_array'][-1], turker_time, busy_time=TIME_IN_MECHANISM):
            mechanism = np.random.choice(
                range(len(mechanism_super_dictionary.keys())), p=probabilities)
...
def simulate_experiment_functionalized(filename, LABEL, mechanism_super_dictionary, radius_parameters, \
 LIMIT=100, TIME_IN_MECHANISM=6, WINDOW_LEN=12, TURKERS_PER_WINDOW=3, deficit_offset=0, \
  load_people_from_file=False, filename_forloadingpeople=None, \
   superdictionary_forloadingpeople=None, deficit_offset_forloadingpeople=None,\
   make_deficit_ideal_neg_infin = False, addeducationoffset = False, onesided = False, quadratic = False):
    ...
    if load_people_from_file:
        ...
        make_deficit_ideal_neg_infin = make_deficit_ideal_neg_infin, addeducationoffset = addeducationoffset);
    ...
    with open(filename, 'wb') as f:
        ...
        for turker_time in turker_times:
            ...
            row = do_mechanism(turker_idealpt, mechanism_assignment, mechanisms, time_of_last_turker, \
             turker_time, mechanism_super_dictionary, deficit_offset, radius_parameters, current_avg_number, onesided = onesided, quadratic = quadratic)
            writer.writerow(row);
