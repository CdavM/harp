...
def assignment_david_option2(
	...
	for mech in mechanism_super_dictionary.keys():
		all_busy = all_busy and check_busy(mech, time, time_of_last_turker, busy_time = TIME_IN_MECHANISM)
...
def sample_in_ball(radius, num_dimensions = 4):
	...
	while length_of_vector > 1:
		...
		length_of_vector = sum([math.pow(vector[i],2) for i in range(num_dimensions)]);
